...
class AI_Algorithm:
			...
			for col in range(len(self.board)):
				if self.board[row][col] == 0:
					...
					for drow, dcol in directions:
						...
						if  (count == 4 and open_ends >= 0) or adjacent_two == 2 or (count == 3 and open_ends == 2):
							if count == 3 and open_ends >= 1:
								can_win_in_two_moves = True
							else:
								winning_moves.append((row, col)) #a winning move is a move which directly leads to a win (not just a (small) chance to win)
							break
		if winning_moves:
			log_info_overruling("It will choose on its own, it can win in a few (1 or 2) moves if it does the right move.")
			return True
		else:
	...
	def get_valid_moves(self)->list:
		...
		if overrule and self.threat_moves: #if threat_moves is not empty (=don't overrule when there are no threats)
			if not self.can_win_in_a_few_moves(opponent_can_win_in_one_move): #don't overrule when AI can win in one move
				...
				return self.execute_normal_move()
		else:
			return self.execute_normal_move()

	def execute_normal_move(self):
